  ...
  public String getData(int idx) {
    String data =
        idx == 0 || idx < tokens.length
            ? tokens[idx]
            : null; // return null for trailing values that are not specified
        ...
        if (c == GENOTYPE_PHASED || c == GENOTYPE_UNPHASED || j == nrGenotypeChars - 1) {
          if (j - start == 1) {
            ...
            if (alleleChar != '.') {
              ...
              if (alleleIndex == 0) cachedAlleles.add(referenceAllele);
              else cachedAlleles.add(alternateAlleles.get(alleleIndex - 1));
            } else {
              cachedAlleles.add(Allele.ZERO);
            }
          } else {
            String alleleIndexStr =
                j == nrGenotypeChars - 1 ? genotype.substring(start) : genotype.substring(start, j);
            if (!alleleIndexStr.equals(".")) {
              ...
              try {
                ...
                alleleIndex =
                    Integer.parseInt(CharMatcher.JAVA_ISO_CONTROL.removeFrom(alleleIndexStr));
              }
              if (alleleIndex == 0) cachedAlleles.add(referenceAllele);
              else cachedAlleles.add(alternateAlleles.get(alleleIndex - 1));
            } else {
